...
import org.hildan.games.blackbox.model.propagation.Propagator;
...
public class Grid {
    ...
    /**
     * Shoots a ray from every port. After a call to this method, no port can be in the
     * {@link State#UNKNOWN} state .
     */
    ...
    /**
     * Shoots a ray from the specified {@link Side}, at the specified position on this side.
     *
     * @param side
     *            The {@code Side} to shoot from.
     * @param index
     *            The index of the port within the specified {@code Side}.
     * @return The twin of the tested port if the result is a {@link State#DETOUR}, {@code null}
     *         otherwise.
     */
    ...
    /**
     * Returns a {@code String} representation of the ports of the specified {@link Side}.
     *
     * @param side
     *            The {@code Side} of the {@code EntryPort}s to display.
     * @return a {@code String} representation of the ports' states of the specified {@code Side}.
     */
    ...
    protected String toString(BallList ballsToDisplay) {
        ...
        sb.append(newLine);
        sb.append(Drawing.repeat(HD, size, 2, "", ULC + Drawing.repeat(H, 2) + CROSS, CROSS + Drawing.repeat(H, 2)
                + URC));
        sb.append(newLine);
        for (int i = 0; i < size; i++) {
            sb.append(V);
        ...
        sb.append(Drawing.repeat(HD, size, 2, "", DLC + Drawing.repeat(H, 2) + CROSS, CROSS + Drawing.repeat(H, 2)
                + DRC));
        sb.append(newLine);
        sb.append("   ");
